...
echo -e "${CYAN}"
echo "#########################################################"
echo "#      Start downloading and installing  freeserver      #"
echo "#########################################################"
echo -e "${NC}"

IPK_URL="https://raw.githubusercontent.com/Ham-ahmed/210/refs/heads/main/enigma2-plugin-extensions-levi45-freeserver_1.8_all.ipk"
IPK_NAME="enigma2-plugin-extensions-levi45-freeserver_1.8_all.ipk"
IPK_PATH="/var/volatile/tmp/$IPK_NAME"

echo -e "${YELLOW}Downloading the freeserver...${NC}"
wget -O $IPK_PATH $IPK_URL

if [ $? -eq 0 ]; then
    echo -e "${GREEN}✓ freeserver has been downloaded successfully....${NC}"
else
    echo -e "${RED}✗ Failed to download freeserver...${NC}"
    exit 1
fi
